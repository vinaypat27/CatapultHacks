...
# 🔎 Search for signature prompts near blank space/underscores
import os
...
def find_signature_mentions(text, page, page_num, output_dir="signature_snips"):
    ...
    for keyword in signature_keywords:
        ...
        for match in results:
            ...
            # Must contain: signature line or long blank space
            ...
            if has_signature_line or has_trailing_space or has_colon_spacing:
                # Search for position of the keyword on the page
                keyword_rects = page.search_for(keyword)
                for i, rect in enumerate(keyword_rects):
                    expanded_rect = fitz.Rect(rect.x0 - 50, rect.y0 - 20, rect.x1 + 300, rect.y1 + 60)  # widen the box
...
# 📄 Analyze PDF + optionally export as text
def analyze_pdf_for_signatures(pdf_path, output_txt_path=None):
    ...
    # ✍️ Write text to .txt file (optional)
    if output_txt_path:
